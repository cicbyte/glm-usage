#!/usr/bin/env python3
"""本地构建脚本 — 交叉编译 glm-usage 并可选 UPX 压缩"""
...
LDFLAGS_PREFIX = "github.com/cicbyte/glm-usage/cmd"
...
def build_target(goos, goarch, version, commit, build_time):
    ...
    output_name = f"glm-usage_{goos}_{goarch}{ext}"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="glm-usage 本地构建脚本")
    ...
    print(f"glm-usage {version} | commit: {commit[:8]} | {build_time}")
    print()
    ...
    for goos, goarch in targets:
        if not build_target(goos, goarch, version, commit, build_time):
            success = False
